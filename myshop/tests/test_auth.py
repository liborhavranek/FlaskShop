...
class TestAuthRegister(TestMixin, unittest.TestCase):
    ...
    def create_registration_form_data(self):
        ...
        self.form = RegistrationForm(
            ...
            faktura_first_name="John",
            faktura_last_name="Doe",
            faktura_city="New York",
            faktura_street="Main Street",
            faktura_zipcode="37010",
            dodej_first_name="John",
            dodej_last_name="Doe",
            dodej_city="New York",
            dodej_street="Main Street",
            dodej_zipcode="37010",
            dodej_info="3rd floor",
            dodej_phone_code="+420",
            dodej_phone="987654321",
            firma_ico="88888888",
            firma_dic="1234567890",
            firma_bank_acc="0987654321",
            firma_bank_number="0800",
            firma_spec_symbol="1234",
        )
    ...
    def test_auth_register_return_correct_correct_status_code_when_mismatch_passwords(
        self,
    ):
        data = {
            "username": "testuser",
    ...
    def test_register_with_mismatched_passwords_return_correct_message(self):
        ...
        response = self.client.post("/auth/register", data=data, follow_redirects=True)
        self.assertIn(
            bytes("Heslo a potvrzení hesla se musí shodovat.", "utf-8"), response.data
        )
    ...
    def test_registration_form_show_message_when_is_not_validate(self):
        with self.app.test_client() as client:
            ...
            self.assertIn(
                bytes("Tento email je už zaregistrován v naší databázi.", "utf-8"),
                response.data,
            )
    ...
    def test_registration_form_show_message_when_is_not_username_validate(self):
        with self.app.test_client() as client:
            ...
            self.assertIn(
                bytes(
                    "Toto uživatelské jméno už je zaregistrované v naší databázi.",
                    "utf-8",
                ),
                response.data,
            )

    def test_registration_form_dont_save_user_in_the_db_when_is_username_not_validate(
        self,
    ):
        with self.app.test_client() as client:
            self.create_user()
    ...
    def test_registration_form_show_message_when_is_not_password_validate(self):
        with self.app.test_client() as client:
            ...
            response = client.post("auth/register", data=form.data)
            self.assertIn(
                bytes("Heslo musí mít alespoň 8 znaků.", "utf-8"), response.data
            )
    ...
    def test_registration_form_show_message_when_is_not_faktura_zipcode_validate(self):
        with self.app.test_client() as client:
            ...
            response = client.post("auth/register", data=form.data)
            self.assertIn(
                bytes("PSČ musí mít přesně 5 číslic.", "utf-8"), response.data
            )
