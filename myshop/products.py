...
def create_brand():
    ...
    return render_template(
        "add_brand.html", form=form, brands=brands, customer=current_user
    )
...
def edit_brand(id):
    ...
    return render_template(
        "edit_brand.html", brand=brand, form=form, customer=current_user
    )
...
def create_category():
    ...
    return render_template(
        "add_category.html", form=form, categories=categories, customer=current_user
    )
...
def edit_category(id):
    ...
    if form.validate_on_submit():
        ...
        flash("Značka byla aktualizována.", category="success")
    return render_template(
        "edit_category.html", category=category, form=form, customer=current_user
    )
...
def product_list():
    ...
    return render_template(
        "product-list.html", products=products, customer=current_user
    )
    ...
    if main_image_form.validate_on_submit():
        ...
        if product_image:
            ...
            if existing_image_filename:
                os.remove(
                    os.path.join(
                        current_app.config["UPLOAD_FOLDER"], existing_image_filename
                    )
                )
    ...
    if additional_images_form.validate_on_submit():
        ...
        for additional_image in additional_images:
            if additional_image.filename != "":
                ...
                new_image_filename = save_image(
                    additional_image, current_app.config["UPLOAD_FOLDER"]
                )
                additional_image_filenames.append(new_image_filename)

        for filename in additional_image_filenames:
            product_image = ProductImage(image_name=filename, product_id=product_id)
...
def edit_product(product_id):
    ...
    return render_template(
        "edit_mobile_product.html", product=product, form=form, customer=current_user
    )
...
def delete_mobile_product(id):
    ...
    main_image = os.path.join(
        current_app.config["UPLOAD_FOLDER"], product.product_image
    )
...
def create_notebook_product():
    ...
    return render_template(
        "add_notebook_product.html", form=form, customer=current_user
    )
...
def edit_notebook_product(product_id):
    ...
    return render_template(
        "edit_notebook_product.html", product=product, form=form, customer=current_user
    )
...
def edit_console_product(product_id):
    ...
    return render_template(
        "edit_console_product.html", product=product, form=form, customer=current_user
    )
...
def create_smart_watch_product():
    ...
    return render_template(
        "add_smart_watch_product.html", form=form, customer=current_user
    )
