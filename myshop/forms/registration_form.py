...
class RegistrationForm(FlaskForm):
    ...
    username = StringField(
        "Přihlašovací jméno:", validators=[DataRequired(), Length(min=5, max=30)]
    )
    email = StringField(
        "Email:", validators=[DataRequired(), Length(min=10, max=50), Email()]
    )
    phone_code = SelectField("Kód:", choices=[("+420", "+420"), ("+421", "+421")])
    phone = StringField(
        "Telefon:",
        validators=[DataRequired(), Length(min=9, max=9), Regexp("^[0-9]*$")],
    )
    password = PasswordField("Heslo:", validators=[DataRequired(), Length(min=8)])
    confirm_password = PasswordField(
        "Potvrdit heslo:", validators=[DataRequired(), EqualTo("password")]
    )
    ...
    dodej_phone = StringField(
        "Telefon:", validators=[Optional(), Length(min=9, max=9), Regexp("^[0-9]*$")]
    )
    ...
    firma_bank_acc = StringField(
        "Číslo účtu:", validators=[Optional(), Length(min=8, max=20)]
    )
    firma_bank_number = StringField(
        "Kód banky:", validators=[Optional(), Length(min=4, max=4)]
    )
    firma_spec_symbol = StringField(
        "Specifický symbol:", validators=[Optional(), Length(min=4, max=20)]
    )
    ...
    def validate_username(self, username):
        ...
        if customer:
            flash(
                "Toto uživatelské jméno už je zaregistrované v naší databázi.", "error"
            )
